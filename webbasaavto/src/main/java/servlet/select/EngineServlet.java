...
@WebServlet("/views/getallavtobyengine")
public class EngineServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try(Connection connection = ConnectionUrl.createConnection();
            PreparedStatement statement = connection.prepareStatement("SELECT * from 'avto' WHERE engine = ? ORDER BY year;");){

            int  engineparam = Integer.parseInt(req.getParameter("engine"));
            statement.setInt(1, engineparam);
            ...
            if(a==0){
                ...
                req.setAttribute("avto_notyear", "Авто такого объема двигателя в базе нет");
            }
            else{
